 ...
 */
export interface AtlasRuleMetadata {
  rule: Rule | null;
  title: string;
  description: string;
  canonicalUrl: string;
 ...
 *
 * Called from ``generateMetadata`` on the server so Atlas rule URLs
 ...
 */
export async function getAtlasRuleMetadata(
  segments: string[] | undefined
): Promise<AtlasRuleMetadata> {
  const citationPath = (segments ?? []).join("/");
  const canonicalUrl = citationPath
    ? `${SITE_URL}/atlas/${citationPath}`
    : `${SITE_URL}/atlas`;
  ...
  if (!rule) {
    return {
      rule: null,
      title: `${citationPath} — Atlas`,
      description:
        "This citation is not yet ingested in the Atlas. View other atomic rules in the encoded legal code.",
      canonicalUrl,
      citationPath,
      jurisdiction: null,
      docType: null,
  ...
  const title = heading
    ? `${citationLabel} — ${heading} · Atlas`
    : `${citationLabel} · Atlas`;
  const description = body
    ? firstSentence(body)
    : heading
      ? `Atomic rule: ${heading}. Source text and encoding in Atlas.`
      : `Atomic rule at ${citationLabel} — view source and encoding in Atlas.`;
 ...
 */
export function buildLegislationJsonLd(meta: AtlasRuleMetadata): object | null {
